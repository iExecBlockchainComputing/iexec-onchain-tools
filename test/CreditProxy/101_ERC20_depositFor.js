...
contract('CreditProxy - Deposit For', async (accounts) => {
	...
	it("Proxy DepositFor - failure (not approved)", async () => {
		...
		await shouldFail.reverting(CreditProxyInstance.depositFor(rlc_deposit, user, { from: iexecAdmin, gas: constants.AMOUNT_GAS_PROVIDED }));
	...
	it("Proxy DepositFor - failure (not authorized)", async () => {
		...
		await shouldFail.reverting(CreditProxyInstance.depositFor(rlc_deposit, user, { from: user, gas: constants.AMOUNT_GAS_PROVIDED }));
